function [x_value, y_value, x1_value, y1_value] = error_fourth_her(n)
 %Double precision
    % Parameters
    epsil = 1e-10;
    Nn = floor(n/2);
    ...
    % Extended precision fourth order method
digits(32)
n = vpa(n);
epsil = vpa((10)^(-11));
Nn = floor(n/2);
...
xp = vpa(0);
    ...
    while i < Nn + ind
        ...
        delta = vpa(pi)/vpa(sqrt(2*n+1 - xn^2));
        xp = xn;
        xn = xp + delta;
    end
